...
\citep{mukherjee_relativistic_2016}}\hspace{0pt}%DIFAUXCMD
        ...
        \begin{figure}
            ...
            \begin{subfigure}{0.3\textwidth}
                ...
                \label{fig:3C433}
            \end{subfigure}
            \caption[Three radio galaxies with interesting structure.]{Radio galaxies, displayed with an arcsinh colour scale. All images were taken with the VLA. (a) is a narrow-angled tail radio galaxy \citep{leahy_atlas_nodate}, (b) is an X-shaped radio galaxy \citep{leahy_polarization_1986}, and (c) is a very \DIFdelbeginFL \DIFdelFL{unusually-shaped }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{unusually shaped }\DIFaddendFL radio galaxy \citep{black_study_1992}.}
...
\section{Data}\label{sec:atlas-xid-data}
  ...
  \subsection{Binary \DIFdelbegin \DIFdel{Classifiers}\DIFdelend \DIFaddbegin \DIFadd{classifiers}\DIFaddend }\label{sec:atlas-xid-classifiers}
    ...
    \DIFdelbegin \DIFdel{biologically-inspired }\DIFdelend \DIFaddbegin \DIFadd{biologically inspired }\DIFaddend prediction models with image inputs.
...
\section{Discussion}
  ...
  compact or \DIFdelbegin \DIFdel{partially-resolved }\DIFdelend \DIFaddbegin \DIFadd{partially resolved }\DIFaddend objects in ATLAS. This result shows that for
  compact and \DIFdelbegin \DIFdel{partially-resolved objects}\DIFdelend \DIFaddbegin \DIFadd{partially resolved objects, }\DIFaddend methods that do not use machine
...
\DIFdel{. We limit }\DIFdelend \DIFaddbegin \aref{sec:rlfs-rlf-desc}\DIFadd{. We limited }\DIFaddend our sample to radio sources with
  ...
  \end{equation}
  where $z$ is redshift and $d$ is luminosity distance (a function of $z$). \DIFaddbegin \DIFadd{The average $k$-correction was $0.92$.
  }\DIFaddend Uncertainties in comoving density \DIFdelbegin \DIFdel{are }\DIFdelend \DIFaddbegin \DIFadd{were }\DIFaddend estimated as described in \DIFdelbegin %DIFDELCMD < \autoref{sec:rlfs-rlf-desc}%%%
...
\section{Discussion}\label{sec:rlfs-discussion}
  ...
  \subsection{Biases and uncertainties}
    ...
    Our requirement for radio emission to be extended \DIFdelbegin \DIFdel{will miss }\DIFdelend \DIFaddbegin \DIFadd{misses }\DIFaddend radio galaxies that would be resolved and extended if they were not aligned with the \DIFdelbegin \DIFdel{line of sight}\DIFdelend \DIFaddbegin \DIFadd{line-of-sight}\DIFaddend . We therefore must be underestimating the population of extended sources (though assuming a random distribution of orientations, the majority of galaxies \DIFdelbegin \DIFdel{will not be }\DIFdelend \DIFaddbegin \DIFadd{are not }\DIFaddend aligned close to the \DIFdelbegin \DIFdel{line of sight}\DIFdelend \DIFaddbegin \DIFadd{line-of-sight}\DIFaddend ). The requirement for extended radio emission \DIFdelbegin \DIFdel{will also impose }\DIFdelend \DIFaddbegin \DIFadd{also imposes }\DIFaddend a lower limit on linear size, which \DIFdelbegin \DIFdel{will vary }\DIFdelend \DIFaddbegin \DIFadd{varies }\DIFaddend with redshift: \DIFdelbegin \DIFdel{at }\DIFdelend \DIFaddbegin \DIFadd{At }\DIFaddend $z = 0.6$ the effect \DIFdelbegin \DIFdel{will be }\DIFdelend \DIFaddbegin \DIFadd{is }\DIFaddend strongest and we \DIFdelbegin \DIFdel{will see no }\DIFdelend \DIFaddbegin \DIFadd{cannot see }\DIFaddend sources with linear size under $33.5$ kpc. This \DIFdelbegin \DIFdel{will cause }\DIFdelend \DIFaddbegin \DIFadd{causes }\DIFaddend us to underestimate the population of radio galaxies with linear sizes between $10$--$30$ kpc. On the other hand, we \DIFdelbegin \DIFdel{have likely avoided }\DIFdelend \DIFaddbegin \DIFadd{likely avoid }\DIFaddend significant overestimation of radio luminosity due to relativistic beaming, since we filter out sources aligned along the \DIFdelbegin \DIFdel{line of sight}\DIFdelend \DIFaddbegin \DIFadd{line-of-sight}\DIFaddend .
    ...
    estimate uncertainty in our algorithm, though in future we anticipate that we \DIFdelbegin \DIFdel{can
    }\DIFdelend \DIFaddbegin \DIFadd{will be able to
    }\DIFaddend employ an ensemble of classifiers to estimate this \citep[e.g.][]{lakshminarayanan17uncertainty}.
...
. We performed the
  following steps:
    ...
    \item \DIFadd{Remove all radio sources that do not fit the selection criteria.
      ...
      We account for the fact that FIRST does not cover the
...
.
  ...
  \hline
    ...
    \DIFadd{This appendix describes our search for giant radio galaxies in RGZ-Ex, and the results of this search. To identify radio sources we assumed that if any two components had the same host galaxy then they were part of the same source. This would be a reasonable assumption if all host galaxies were correctly identified, which was not the case. This assumption therefore introduced spurious sources due to galaxies that were incorrectly identified as host galaxies: Not all sources used in this chapter are real sources, and in particular sources of large angular size are likely to be incorrect. Nevertheless RGZ-Ex provides a useful catalogue of }\emph{\DIFadd{candidate}} \DIFadd{radio sources, and visual follow-up can confirm whether sources of interest are real.
}
...
mostly due to sidelobes/artefacts (103), incorrect source grouping (82), or incorrect SDSS matches (21). The citizen scientists who identified giants were WizardHowl, DolorousEdd, antikodon, csunjoto, sisifolibre, JeanTate, JKD, PADV, and firejuggler. H.A., together with his summer students, had previously identified 29 of these giants.
    ...
    \DIFadd{Note that this is a particularly challenging set: Sources that were misidentified often had unusually large estimated extents due to the inclusion of spurious components. The error rate in this set therefore does not reflect the rest of the catalogue.
}
...
\DIFaddbegin 
    ...
    Our RLFs could be improved. The RLF calculations in \autoref{cha:rlfs} are severely limited by the availability of redshifts. We \DIFdelbegin \DIFdel{limit }\DIFdelend \DIFaddbegin \DIFadd{limited }\DIFaddend our analyses to host galaxies that \DIFdelbegin \DIFdel{do }\DIFdelend \DIFaddbegin \DIFadd{did }\DIFaddend have available spectroscopic redshifts in SDSS, but we could also \DIFdelbegin \DIFdel{employ }\DIFdelend \DIFaddbegin \DIFadd{have employed }\DIFaddend the less-reliable but considerably more prolific photometric redshifts. These are derived from regression models rather than direct observations of redshifted spectral lines, and so can be produced from photometric surveys without dedicated spectroscopy. However, without methods to handle the uncertainty introduced by photometric redshifts, the resulting RLFs would be unreliable. \DIFdelbegin \DIFdel{Developing ways to not only decrease }\DIFdelend \DIFaddbegin \DIFadd{Decreasing }\DIFaddend the uncertainty in photometric redshifts \DIFdelbegin \DIFdel{but also to understand and incorporate the uncertainty }\DIFdelend \DIFaddbegin \DIFadd{is not the only way forward. We may also develop methods for understanding and incorporating their uncertainties }\DIFaddend into downstream calculations like those \DIFdelbegin \DIFdel{of the }\DIFdelend \DIFaddbegin \DIFadd{used for }\DIFaddend RLFs, for example\DIFaddbegin \DIFadd{, }\DIFaddend using probabilistic programming \citep[e.g.][]{bingham2018pyro}\DIFdelbegin \DIFdel{, }\DIFdelend \DIFaddbegin \DIFadd{. This }\DIFaddend will allow these photometric redshifts to be used and tremendously increase sample sizes. Besides existing photometric redshifts, future surveys will also produce many more redshifts, both spectroscopic and photometric. This will be very important for surveys like EMU, which are both deep and wide with low redshift availability.
