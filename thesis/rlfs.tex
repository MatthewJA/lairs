...
\label{sec:rlfs-rlf}
  ...
  from the RGZ-Ex catalogue. We first eliminate sources and components with high sidelobe probability according to the FIRST catalogue and low BXID scores (see \aref{sec:rlfs-sankey} for a diagram of this filtering process). To calculate each RLF we 
  ...
  describe this approach in \aref{sec:rlfs-rlf-desc}. We limit our sample to radio sources with
  ...
  Uncertainties in comoving density are estimated as described in \aref{sec:rlfs-rlf-desc}. Completeness estimates are shown in \aref{sec:rlfs-completeness}. We discuss biases in our methods and results in \autoref{sec:rlfs-bias}.
...
\label{sec:rlfs-rlf-desc}
  ...
  \begin{enumerate}
    ...
    \item The count for each luminosity bin is the sum over $1 / V_{\max}$ for
      each source in the bin. We divided these counts by the estimated completeness (\aref{sec:rlfs-completeness}) to account for redshift incompleteness.
